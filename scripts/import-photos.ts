...
const albumTags = (title: string, tags?: string | null) => tags || (title.includes('세례식') ? '세례식' : null)
...
async function main() {
  ...
  try {
    ...
    for (const [title, sourceItems] of albums) {
      ...
      const result: any = await client.query('INSERT INTO photo_albums (title, slug, legacy_key, description, tags, event_date, cover_image_url, image_count) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (legacy_key) DO UPDATE SET title=EXCLUDED.title, description=EXCLUDED.description, tags=EXCLUDED.tags, event_date=EXCLUDED.event_date, cover_image_url=EXCLUDED.cover_image_url, image_count=EXCLUDED.image_count, updated_at=now() RETURNING id', [title, 'album-' + hash(title), title, first.description ?? null, albumTags(title, first.tags), newestDate, r2Path(first.img_path), items.length])
