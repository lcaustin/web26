...
export async function GET(request: Request) {
  ...
  if (!ids.length) return new Response('No groups selected', { status: 400 })

  const result = await payload.find({ collection: 'bible-studies', where: { id: { in: ids } }, limit: ids.length })
